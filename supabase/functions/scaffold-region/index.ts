  ...
  return {
    ...
    where: {
      map: {
        ...
        zoom: 7,
        markers: [],
        // Baseline overlay set — every region ships with these four toggleable
        // layers. Research fills `features`; layers render off by default so
        // they never overlap unless the reader turns one on.
        // NOTE: do NOT add a "towns" overlay here — town/city pins are rendered
        // from `markers` under the built-in "Cities & Towns" toggle.
        overlays: [
